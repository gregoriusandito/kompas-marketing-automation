let manifest_url            =   kompas_automation_global_vars.home_url + '/manifest.json',
    service_worker_url      =   kompas_automation_global_vars.home_url + '/service-worker.js',
    surrogate_key           =   null,
    relative_path           =   null,
    custom_event            =   null;
...
jQuery.when(manifest_check, sw_check, get_surrogate_key)
    .done((manifest_check, sw_check, get_surrogate_key) => {
        ...
        */
        ...
        if( surrogate_key ) {
            appboy.changeUser(surrogate_key);  
        } 
        
        // if custom event exist, activate logCustomEvent
        // default: send current url
        if ( custom_event ) {
            appboy.logCustomEvent( custom_event, {url: kompas_automation_global_vars.this_url} );    
        }
